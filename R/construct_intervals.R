#' Construct intervals for linear mixed effects
#'
...
#' @param alpha Alpha level to cosntruct intervals
#' @param n_boot Number of bootstrap samples, default n_boot = 1000
#' @param boot_seed Vector with cluster labels
#' @param ... Additional parameters
#'
#' @return List with individual and simultaneous intervals, and their length
#'
#' @export
#'
#' @examples
...
construct_intervals <- function(type_method = c("parametric",
  ...
  intervals <- construct_intLME(model_obj = model_type,
...
#' Intervals for linear mixed models
#'
...
construct_intLME.NERM <- function(model_obj,
  ...
  fitted_NERM <- fit_NERM(formula_y, data_sample, id_cluster, cluster_means)
  ...
  if (type_method == "analytical") {
    ...
    int_up <- fitted_NERM$mu_hat + sqrt(fitted_NERM$mse) *  q_ind_alpha
    int_down <- fitted_NERM$mu_hat - sqrt(fitted_NERM$mse) *  q_ind_alpha

    q_sim_alpha = qnorm(1 - alpha / (2 * m))
    int_sim_up <- fitted_NERM$mu_hat + sqrt(fitted_NERM$mse) *  q_sim_alpha
    int_sim_down <- fitted_NERM$mu_hat - sqrt(fitted_NERM$mse) *  q_sim_alpha
    ...
    mu_matrix <- matrix(0, ncol = length(boot_samples$fit_boot_NERMS[[1]]$mu_hat),
                        nrow = length(boot_samples$fit_boot_NERMS))

    mu_hat_matrix <- matrix(0, ncol = length(boot_samples$fit_boot_NERMS[[1]]$mu_hat),
                            nrow = length(boot_samples$fit_boot_NERMS))

    sd_mixed <- matrix(0, ncol = length(boot_samples$fit_boot_NERMS[[1]]$mu_hat),
                            nrow = length(boot_samples$fit_boot_NERMS))
    ...
    rmse_mixed <- boot_MSE(type_var_estimator, boot_samples,
      cluster_means, var_u_est, var_e_est)$rmse_mixed
    ...
    q_sim_alpha = quantile(apply(dif_M, 1L, max) , prob = 1 - alpha, type = 8)
    q_ind_alpha = apply(dif_t, 2L, quantile, probs = c(alpha / 2, 1 - alpha / 2))


    if (type_var_estimator == "g1") {
      int_up <- fitted_NERM$mu_hat + sqrt(fitted_NERM$g1) *  q_ind_alpha[2]
      int_down <- fitted_NERM$mu_hat + sqrt(fitted_NERM$g1) *  q_ind_alpha[1]
      length_int <- int_up - int_down

      int_sim_up <- fitted_NERM$mu_hat + sqrt(fitted_NERM$g1) * q_sim_alpha
      int_sim_down <- fitted_NERM$mu_hat - sqrt(fitted_NERM$g1) * q_sim_alpha
  ...
  output <- list(int_up = int_up, int_down = int_down, length_int = length_int,
                 int_sim_up = int_sim_up, int_sim_down = int_sim_down,
