...
#' \item{fit_double_boot_NERMS}{fitted NERM models to doubl-bootstrap samples}
#'
#' @export
#'
#' @examples
...
bootstrap_NERM.parametric <- function(boot_object,
  ...
  generate_samples <- generate_NERM(generate_u = list(type = "normal",
  ...
  if (type_var_estimator == "mse_bc") {
    ...
    fit_double_boot_NERMS <- lapply(generate_db_samples, NERMS_wrapper)
...
bootstrap_NERM.semiparametric <- function(boot_object,
  ...
  # Fix beta_est and beta_hat, decide on one of them.
  generate_samples <- generate_NERM_sp(empirical_u = empirical_u,
                                       var_u_est = var_u_est,
  ...
  if (type_var_estimator == "mse_bc") {
    ...
    db_wrapper <- function(estimated_NERM_db) {
      generate_samples <- generate_NERM_sp(empirical_u = estimated_NERM_db$u_hat,
